...
export const checkPhone = (tel) => {
    if ((/^0\d{2,3}-?\d{7,8}$/.test(tel) || /^1(3|4|5|6|7|8|9)\d{9}$/.test(tel) || /^400[0-9]{7}/.test(tel))) {
        return true
    } else {
        return false
    }
...
// 绑定桌码
export const scanCode = (storesId, orderId) => {
    return new Promise((resolve, reject) => {
        postMessage('scan', 'homeScan')
        onMessage("homeScan", async (data) => {
            let options = parseQueryString(decodeURIComponent(data)) // 正式线上版本
            // let path = res.result // ty版本 模拟二维码
            ...
            if (path.split(',')[0] == storesId && path.split(',')[1]) {
                let tableId = path.split(',')[1]
                let res2 = await Axios.get('/order/bind', {
                    tableId: tableId,
                    orderId: orderId,
                    storesId: storesId
                })
...
// 扫码点餐
export const goCheckGoodsByScan = () => {
    postMessage('scan', 'homeScan');
    onMessage("homeScan", data => {
        let options = parseQueryString(decodeURIComponent(data)) // 正式线上版本
        if(options.scene){
            goPage(`/checkGoods?scene=${options.scene}`);
        }else{
            Toast.fail('无效码')
        }
    })
