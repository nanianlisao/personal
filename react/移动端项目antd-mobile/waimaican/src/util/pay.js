...
// 外卖下单并且支付
export const payTakeOut = (data = {}) => {
    return new Promise(async (resolve, reject) => {
        try{
            if (window.env == 'ali') {
                let res1 = await Axios.post('/order/ali/create/pay', data)
                if (res1.data.resultCode == 0) {
                    if (res1.data.payOrderAliappletVo) {
                        window.my.tradePay({
                            tradeNO: res1.data.payOrderAliappletVo.tradeNO,
                ...
                let res1 = await Axios.post('/order/create/pay', data)
                if (res1.data.resultCode == 0) {
                    if (res1.data.payOrderWcappletVo) {
                        let payOrderWcappletVo = res1.data.payOrderWcappletVo
                        postMessage('pay', payOrderWcappletVo)
...
export const dinnerPay = (data = {}) => {
    ...
    return new Promise(async (resolve, reject) => {
        if (window.env == 'ali') {
            ...
            let res = await Axios.post('/order/dinner/pay', {
                orderIds: orderId.split(','),
                ticketId: ticketId
            })
            if (res.data.resultCode == 0) {
