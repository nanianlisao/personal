...
export default class NoticeBar extends Component {
    ...
    load() {
        ...
        clearTimeout(this.state.timer3)

        this.state.timer1 = setTimeout(fun1, 1000);
        function fun1() {
            if (p_w > wrapper.scrollLeft) {
                wrapper.scrollLeft++;
                that.state.timer2 = setTimeout(fun1, that.props.fps);
            }
            else {
                that.state.timer3 = setTimeout(fun2, 1000);
            }
        }
        function fun2() {
