      ...
      return util.wx_requestGet(url + path, {
        ...
        return new Promise(function(resolve, reject) {
          if (res.data.resultCode == 1) {
            app.globalData.cache.set(key, new CacheWrapper(new Date().getTime(), res.data, data.timess));
            resolve(res.data);
          } else {
            reject({
  ...
  requestPost(path, data) {
    ...
    if (app.globalData.cache.has(key) && !app.globalData.cache.get(key).isTimeOut() && (!data.noCache)) {
      ...
      return util.wx_requestPost(url + path, {
        ...
        return new Promise(function(resolve, reject) {
          if (res.data.resultCode == 1) {
            app.globalData.cache.set(key, new CacheWrapper(new Date().getTime(), { ...res.data
            }, data.timess));
            resolve(res.data);
          } else {
            reject({
