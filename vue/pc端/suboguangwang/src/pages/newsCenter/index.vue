...
export default {
  ...
  methods: {
    ...
    onJumpDetail(id) {
      this.$router.push(
        `/newsCenter/newsCenterDetail/${id}?activeBar=${this.activeBar}&subColumnId=${this.subColumnId}&year=${this.year}`
      );
    },
    onChangePage(page) {
      this.page = page;
    ...
    // 下一页
    onPrev() {
      let last = this.yearList[this.yearList.length - 1]; // 找到最后一个年
      let arr = [];
      for (let i = 0; i < 8; i++) {
        let year = --last;
        if (year < 2013) {
      ...
      if (arr.length < 8) {
        let arr2 = [...this.yearList.slice(arr.length), ...arr];
        this.yearList = arr2;
      } else {
        this.yearList = arr;
      }
    ...
    onSelectMenu(opt, idx) {
      this.$router.replace(`/newsCenter?subColumnId=${opt}&activeBar=${idx}`);
